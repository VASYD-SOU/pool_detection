...
ap.add_argument("-p", "--path", required=True, help="path to folder with images")
...
for imagePath in imagePaths:
    ...
    if(height==1500 and width==1500):
        ...
        y_scale = height/img.shape[0]
        x_scale = width/img.shape[1]
        ...
        if len(boxIdxs) > 0:
            ...
            for i in boxIdxs:
                ...
                big_box = (int(np.round(startX*x_scale)), int(np.round(startY*y_scale)), 
                int(np.round(endX*x_scale)), int(np.round(endY*y_scale)))
                startX_list.append(big_box[0])
                startY_list.append(big_box[1])
                endX_list.append(big_box[2])
                endY_list.append(big_box[3])
                path_list.append(imagePath)

box_df = pd.DataFrame(list(zip(startX_list, startY_list, endX_list, endY_list, path_list)), columns =['startX', 'startY', 'endX', 'endY', 'path'])
box_df.to_csv(r'P:\Workspace\pool_detection\pool_boxes_075.csv', index=False)

# Kolla om pool_boxes_06 innehåller mer eller mindre än 2861!!!!